 ...
 */
public class JdkConnectionHttpClient implements HttpClient {

    private int connectTimeout = 10_000;
    private int readTimeout = 60_000;


    public JdkConnectionHttpClient() {
    }
    ...
    private HttpResponse doReq0(HttpRequest httpRequest) throws IOException {
        ...
        try {
            ...
            String traceId = "";
            List<String> xtraces = conn.getHeaderFields().get("x-trace-id");
            if (xtraces != null && xtraces.size() > 0) {
                traceId = xtraces.get(0);
            }
            return new HttpResponse(responseCode, readAllBytes(conn.getInputStream()), traceId);
        } finally {
            conn.disconnect();
        }
    }
